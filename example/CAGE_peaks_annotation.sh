...
cut -f 1,$Annotated_DIM ${OPATH}${CAGE_BED_OUT%.bed*}_annotated.txt |  awk 'NF==2' | awk 'BEGIN{OFS="\t"};{print $2, $1}' > ${RES_FILE%.txt*}_0_GeneName.txt
awk 'BEGIN {OFS="\t"};FNR==NR{a[$1]=$2;next}{ print $0, a[$1]}'  ${RES_FILE%.txt*}_0_GeneName.txt $ANN_FILE |  awk 'NF==3' | awk 'BEGIN{OFS="\t"};{print $3,$2,$1}' > ${TEMP}${number_of_file}_$(basename -- ${FILE%.txt*}_${n_len}bp)_GeneName.txt
fi;
fi;
done;
done; 
...
Annotated_DIM=$(( $(awk '{print NF}' ${OPATH}${CAGE_BED_OUT%.bed*}_annotated.txt | sort -nu | tail -n 1) - 1 ))
cut -f 1,$Annotated_DIM ${OPATH}${CAGE_BED_OUT%.bed*}_annotated.txt |  awk 'NF==2' | awk 'BEGIN{OFS="\t"};{print $2, $1}' > ${RES_FILE%.txt*}_0_GeneName.txt
awk 'BEGIN {OFS="\t"};FNR==NR{a[$1]=$2;next}{ print $0, a[$1]}'  ${RES_FILE%.txt*}_0_GeneName.txt $ANN_FILE |  awk 'NF==3' | awk 'BEGIN{OFS="\t"};{print $3,$2,$1}' > ${TEMP}${number_of_file}_$(basename -- ${BED%.txt*}_${n_len}bp)_GeneName.txt
fi;
fi;
done;
done;
...
if [ "$(awk -v n_row=$number_of_file 'NR == n_row {print $2}' $FILES)" != "" || "$(awk -v n_row=$number_of_file 'NR == n_row {print $2}' $BED)" != "" ]; then
rm -rf ${TEMP}*_0_GeneName.txt
for ann in ${TEMP}*_GeneName.txt; do
awk 'BEGIN {OFS="\t"};FNR==NR{a[$1]=$2;next}{ print $0, a[$1]}' ${RES_FILE%.txt*}_CAGEpeaks.txt ${OPATH}${CAGE_BED_OUT%.bed*}_annotated.txt > ${TEMP}Annotation_tmp.txt;
mv ${TEMP}Annotation_tmp.txt ${OPATH}${CAGE_BED_OUT%.bed*}_annotated.txt
sed  -i "1s/.*/GeneName/" ${OPATH}${CAGE_BED_OUT%.bed*}_annotated.txt
done;
fi
